...
def process_batch(
    ...
    for tools, conv in zip(batched_tools, batched_conv):
        ...
        if tools is not None:
            conv.insert(0, {
                "role": "system",
                "content": TOOL_DEFINITION_PREFIX + json.dumps(tools, indent=4,
                                                               ensure_ascii=False)
            })
        ...
        for message in conv:
            ...
            if message['role'] == 'tool':
                ...
                value = FUNCTION_CALL_PREFIX + format_function_call(FUNCTION_CALL_NAME,
                                                                    message["parameters"]) + FUNCTION_CALL_POSTFIX
                new_input_ids = tokenizer.build_single_message("assistant", message["name"], value)
                new_loss_masks = [loss_mask_val] * len(new_input_ids)

                # function call result
                value = message.get('observation', None)
                if not isinstance(value, str):
                    value = json.dumps(value, ensure_ascii=False)
                obs_input_ids = tokenizer.build_single_message("observation", '', value)
                obs_loss_masks = [False] * len(obs_input_ids)

                # adding to the input_ids and loss_masks
                new_input_ids += obs_input_ids
                new_loss_masks += obs_loss_masks
                # raise NotImplementedError()
            else:
...
def process_batch_eval(
    ...
    def add_to_batch(new_input_ids, input_ids):
    ...
    for tools, conv in zip(batched_tools, batched_conv):
        ...
        for message in conv:
            ...
            if message['role'] == 'tool':
                ...
                # adding to the input_ids and output_ids
                new_input_ids += obs_input_ids
                add_to_batch(new_input_ids, input_ids)
                input_ids += new_input_ids
                # raise NotImplementedError()
            else:
                new_input_ids = tokenizer.build_single_message(
